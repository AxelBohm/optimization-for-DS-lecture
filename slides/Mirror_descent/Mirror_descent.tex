...
\begin{frame}
  ...
  \begin{equation}
    \Vert x \Vert = \Big({\sum_{i=1}^{d} x_i^2} \Big)^{1/2}.
  \end{equation}
  In \textbf{gradient descent} we used Lipschitz continuity:
  \begin{equation}
  ...
  \onslide<2->{%
    For \textbf{sub-gradient descent} we used $\Vert g \Vert \le G$ which
...
% put here convergence proof?
%
% add one slide about dual norms
\begin{frame}
  ...
  \onslide<2->{%
    ...
    \begin{equation}
      \D(x, x_1) = \sum x_i \log(\frac{x_i}{\frac1n}) = \sum x_i \log(x_i) + \log(n) \le log(n)
    \end{equation}
  }
