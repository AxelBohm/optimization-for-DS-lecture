...
\begin{frame}
  ...
  \begin{minipage}{0.48\textwidth}
    \begin{block}{$f$ is \emph{smooth} and convex: Gradient Descent}
      % Smoothness means $\Vert \nabla f(x) - \nabla f(y) \Vert \le L \Vert x-y \Vert$:
      \begin{equation}
        \begin{aligned}
          \text{iterate:} \quad x_{k+1} = x_k - \alpha_k \nabla f(x_k) \\
...
\begin{frame}
  ...
  \begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth,keepaspectratio]{subdifferential}
    % \caption{\label{fig:label} }
  \end{figure}
...
\begin{frame}
  ...
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
      ...
      \State{$x_{k+1} = P_C( x_k- \alpha_k g_k )$}
      \Endfor
    \end{algorithmic}
  \end{algorithm}
